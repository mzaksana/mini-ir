...
# HTML::ExtractContent
# util.pl
# Edited: Muammar Zikri Aksana @ 5 Mar 2019 
...
sub saveFile{
    ...
    my $dir= $_[0].filePathFolder(2,dirname($_[1]));
    # concat path dengan nama file
    my $newFile=$dir."/".basename(filePathFolder(1,$_[1]),".html").".bersih.dat";
    ...
    unless(-e $dir or `mkdir -p $dir`) {
        open(my $fail,'>>:encoding(UTF-8)','../../log/logFailedClean.txt') or die;
